...
class Model():
    ...
    def _calculate_loss(
        ...
        fc2_label_list, unsqueeze_reward, 
        unsqueeze_advantage, 
        fc2_value_result, 
        seri_vec, 
        unsqueeze_is_train, 
        ...
        reward, advantage, label_list, is_train, weight_list = self._squeeze_tensor(
            unsqueeze_reward, 
            unsqueeze_advantage, 
            unsqueeze_label_list, 
            unsqueeze_is_train, 
        ...
        for task_index in range(len(self.is_reinforce_task_list)):
            if self.is_reinforce_task_list[task_index]:
                ...
                old_policy_p = tf.reduce_sum(
                    one_hot_actions * old_label_probability_list[task_index], 
                    axis=1)
                ...
                #- tf.reduce_sum(tf.to_float(weight_list[task_index]) * tf.minimum(surr1, surr2)) / tf.maximum(tf.reduce_sum(tf.to_float(weight_list[task_index])), 1.0) # CLIP loss, add - because need to minize
            ...
            if self.is_reinforce_task_list[task_index]:
                ...
                temp_entropy_loss = -tf.reduce_sum(
                    ...
                    * tf.to_float(weight_list[task_index]))
                        ...
                        tf.reduce_sum(tf.to_float(weight_list[task_index])), 
                        1.0)# add - because need to minize
